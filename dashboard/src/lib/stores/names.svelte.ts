/**
 * Name resolution store — rune-based singleton.
 *
 * Batches unknown Discord IDs into a single API call to avoid N+1 fetches.
...
let _userNames = $state<Record<string, string>>({});
let _channelNames = $state<Record<string, string>>({});
	...
	for (let i = 0; i < maxChunks; i++) {
		...
		try {
			...
			if (res.users && Object.keys(res.users).length > 0) {
				_userNames = { ..._userNames, ...res.users };
			}
			if (res.channels && Object.keys(res.channels).length > 0) {
				_channelNames = { ..._channelNames, ...res.channels };
			}
		} catch {
...
export const names = {
	/** Reactive map of resolved user IDs → display names. */
	get users(): Record<string, string> {
		return _userNames;
	},

	/** Reactive map of resolved channel IDs → display names. */
	get channels(): Record<string, string> {
		return _channelNames;
	},

	/**
	 * Request resolution for a set of user/channel IDs.
	 * Returns immediately; read `names.users` / `names.channels` reactively.
	 */
	requestResolve(userIds: string[] = [], channelIds: string[] = []) {
		for (const id of userIds) {
			if (!_userNames[id]) pendingUserIds.add(id);
		}
		for (const id of channelIds) {
			if (!_channelNames[id]) pendingChannelIds.add(id);
		}
